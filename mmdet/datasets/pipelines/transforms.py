...
class RandomCrop(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            offset_h = np.random.randint(0, 256)
            offset_w = np.random.randint(0, 256)  # 手动赋值
        ...
        for key in results.get('bbox_fields', []):
            ...
            if mask_key in results:
                idx = valid_inds.nonzero()[0]
                if len(idx):
                    results[mask_key] = results[mask_key][idx].crop(
                        np.asarray([crop_x1, crop_y1, crop_x2, crop_y2]))
                else:
                    results[mask_key].masks=[]
            results['diffs'] = results['diffs'][valid_inds]
            results['trunc'] = results['trunc'][valid_inds]
...
class Mosaic:  # 大小会增加一倍
    ...
    def _mosaic_transform(self, results):
        ...
        for i, loc in enumerate(loc_strs):
            ...
            gt_masks_i = results_patch['gt_masks'].masks
            diffs_i = results_patch['diffs'].tolist()
            trunc_i = results_patch['trunc'].tolist()
            ...
            if gt_bboxes_i.shape[0] > 0:
                ...
                for i in range(len(gt_masks_i)):
                    gt_masks_i[i][0][0::2] = np.clip(scale_ratio_i * gt_masks_i[i][0][0::2] + padw, 0,
                                                     2 * self.img_scale[1])
                    gt_masks_i[i][0][1::2] = np.clip(scale_ratio_i * gt_masks_i[i][0][1::2] + padh, 0,
                                                     2 * self.img_scale[0])

            mosaic_bboxes.append(gt_bboxes_i)
            mosaic_labels.append(gt_labels_i)
            mosaic_masks += gt_masks_i
            mosaic_diffs += diffs_i
            mosaic_trunc += trunc_i

        if len(mosaic_labels) > 0:  # 如果有目标，限定一下值域
            ...
            mosaic_bboxes, mosaic_masks, mosaic_labels, mosaic_diffs, mosaic_trunc = \
                self._filter_box_candidates(mosaic_bboxes, mosaic_masks, mosaic_labels, np.array(mosaic_diffs),
                                            np.array(mosaic_trunc))
        ...
        results['gt_masks'].masks = mosaic_masks
        results['diffs'] = mosaic_diffs
        results['trunc'] = mosaic_trunc
    ...
    def _filter_box_candidates(self, bboxes, masks, labels, diffs, trunc):
        ...
        return bboxes[valid_inds], [masks[i] for i in valid_inds], labels[valid_inds], diffs[valid_inds], trunc[
            valid_inds]
