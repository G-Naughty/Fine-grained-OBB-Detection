...
def xy_wh_r_2_xy_sigma(xywhr):
    ...
    assert _shape[-1] == 5
    xy = xywhr[..., :2]  #miu
    wh = xywhr[..., 2:4].clamp(min=1e-7, max=1e7).reshape(-1, 2)   #w,h 正数
    r = xywhr[..., 4]
    cos_r = torch.cos(r)
    sin_r = torch.sin(r)
    R = torch.stack((cos_r, -sin_r, sin_r, cos_r), dim=-1).reshape(-1, 2, 2)
    S = 0.5 * torch.diag_embed(wh) # w/2,h/2的对角阵
    # .bmm  矩阵乘法
...
def kld_loss(pred, target, fun='log1p', tau=1.0, alpha=1.0, sqrt=True):
    ...
    Sigma_t_det_log = Sigma_t.det().log()    #   .det求行列式的值; .log自然对数
...
class GDLoss(nn.Module):
    ...
    BAG_PREP = {'xy_stddev_pearson': xy_stddev_pearson_2_xy_sigma,
     ...
     #xy_stddev_pearson
    def __init__(self, loss_type, representation='xy_wh_r',
