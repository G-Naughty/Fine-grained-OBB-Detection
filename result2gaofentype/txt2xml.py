            ...
            if len(bbox_x) <= 5:  # 4,3个点
                ...
                for i in range(np.shape(point)[0]):
                    ...
                    if area > area_max:
                        ...
                        area_max = area
                object_bbox = [float(bbox_now[0][0]), float(bbox_now[0][1]), float(bbox_now[1][0]),
                               float(bbox_now[1][1]),
                               float(bbox_now[2][0]), float(bbox_now[2][1]), float(bbox_now[3][0]),
                               float(bbox_now[3][1])]
            elif len(bbox_x) > 6:
...
txt_path = "/home/hnu1/OBBDetection/work_dir/GGM/" + args.model_name + "/result_txt"
save_path = "/home/hnu1/OBBDetection/work_dir/GGM/" + args.model_name + "/test"
