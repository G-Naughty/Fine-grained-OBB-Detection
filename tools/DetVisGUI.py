...
cfg.data_root = '/home/hnu1/ZDS/ellipse-dota/'
cfg.classes = ['ellipse']
...
cfg.data = dict(
    ...
    val=dict(
        type=cfg.dataset_type,
        ann_file=cfg.data_root + 'test/annfiles/patch_annfile.pkl',
        img_prefix=cfg.data_root + 'test/images/'),
    test=dict(
        type=cfg.dataset_type,
        img_prefix='/home/hnu1/ZDS/ellipse/test/images/'))
...
def parse_args():
    ...
    parser.add_argument('--det_file', default='/home/hnu1/GGM/OBBDetection/work_dir/oriented_obb_ellipse/dets.pkl',
    ...
    def get_img_list(self):  # 根据pkl注释文件获取图片顺序v
        if self.det_file == '':
            ...
            with open(self.det_file, 'rb') as f:
                anns = pickle.load(f)
            ann_list = [ann[0] + '.jpg' for ann in anns]
        return ann_list
